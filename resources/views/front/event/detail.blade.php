...
<ul class="breadcrumb">
  ...
  <div class="detail-page flx-w">
    <div class="center">
    <div class="nahled"><img src="{{asset($event->getMainImage())}}" alt="nahled"></div>
    <h1>{{$event->title}}</h1>
    ...
    <div class="tag" style="background: {{$event->category->backColor}}; color: {{$event->category->foreColor}}">{{$event->category->name}}</div>
      ...
      <li><img src="{{asset('img/front/placered.svg')}}" alt="Místo">{{$event->place->name}}</li>
        @if($event->fb_url)
      <li><img src="{{asset('img/front/sharered.svg')}}" alt="Adresa"><a href="{{$event->fb_url}}">{{$event->fb_url}}</a></li>
        @endif
      </ul>
    </div>
    <div class="left-side">
